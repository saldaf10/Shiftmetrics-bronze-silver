...
| Nombre | Correo |
|---|---|
| Integrante 1 | saldsfishy@gmail.com |
| Integrante 2 | samargo1703@gmail.com |
| Integrante 3 | freddycadavid2015@gmail.com |
| Integrante 4 | juanprc2017@gmail.com |
| Integrante 5 (Juan) | juanmejia0317@gmail.com |

Todos con rol **Owner** en el proyecto GCP `shiftmetrics-analytics`.
Documentación técnica LaTeX: `gs://shiftmetrics-bronze/docs/shiftmetrics_doc.tex`
...
5b. [Paso previo: Conversión BSON → Parquet (Apache Jira)](#5b-paso-previo-conversión-bson--parquet-apache-jira)
...
| Región | `us-central1` |
| Zona | `us-central1-a` |
| Créditos disponibles | $1,098,376.73 (cuenta de prueba gratuita) |
| Bucket Bronze | `gs://shiftmetrics-bronze` (4.36 GiB, 176+ objetos) |
| Bucket Silver | `gs://shiftmetrics-silver` |
| Bucket Gold | `gs://shiftmetrics-gold` |
| Dataset BigQuery | `shiftmetrics-analytics:shiftmetrics_gold` |
| Tabla BigQuery | `shiftmetrics_gold.sprint_features` (42,747 filas) |
| Cluster Dataproc | `shiftmetrics-cluster` — **ELIMINADO** (recrear para ML si se necesita) |
...
## 5b. Paso previo: Conversión BSON → Parquet (Apache Jira)

Apache Jira llegó en Bronze como dump de MongoDB (formato `.bson.gz`). Spark y BigQuery no pueden leer BSON directamente — fue necesario convertirlo a Parquet antes de cualquier procesamiento Silver.

### Infraestructura del job de conversión

| Componente | Detalle |
|---|---|
| Plataforma | Google Cloud Dataproc |
| Framework | Apache Spark 3.x (PySpark) |
| Clúster | `jira-converter` — **single-node** (sin workers) |
| Máquina master | `n1-highmem-8` (8 vCPUs, 52 GB RAM) |
| Imagen | `2.1-debian11` |
| Zona | `us-central1-a` |
| Script | `gs://shiftmetrics-bronze/scripts/bson_fix.py` |

> **Por qué single-node:** La cuota del proyecto es de 12 vCPUs. Un clúster estándar (1 master n1-highmem-8 + 2 workers n1-standard-4) usaría 8+8 = 16 vCPUs — excede la cuota. Single-node usa solo 8 vCPUs.

### Lógica del script `bson_fix.py`

1. Instala `pymongo` al inicio del job vía `subprocess.check_call([sys.executable, "-m", "pip", "install", "pymongo", "-q"])`
2. Descarga el `.bson.gz` completo desde GCS
3. Descomprime y parsea BSON en streaming (int32 LE) — lotes de 3,000 docs
4. Aplana documentos anidados hasta profundidad 2; arrays → string
...
- **Output:** `gs://shiftmetrics-bronze/apache-jira-parquet/`
- Clúster `jira-converter` eliminado inmediatamente tras completar para minimizar costos

### Estructura de carpetas completa en `shiftmetrics-bronze`

```
shiftmetrics-bronze/
├── apache-jira/              ← BSON originales
├── apache-jira-parquet/      ← Parquets convertidos (input de Silver 02)
├── docs/                     ← shiftmetrics_doc.tex (documentación LaTeX)
├── gharchive/                ← 24 archivos JSON.gz
├── promise/                  ← 144 CSVs métricas CK
├── r/
├── redhat-jira/              ← redhat-inputs.zip + redhat-outputs.zip
├── redhat-jira-parquet/      ← 251 parquets (input de Silver 03)
└── scripts/                  ← bson_fix.py, silver_job_*.py, gold_job_*.py
```
